...
```typescript
// ここでは master / web 生成物の変換や Markdown 読み込みなど、
// 現在の通常導線に関わるユーティリティのテストを記述します。
```

master と web 生成物の往復を確認するテストを中心に置き、通常運用の導線に沿ったケースを優先します。
...
デフォルトでは、`npm test` は **Watch モード** で実行されます。これはファイルの変更を監視し、変更があった場合に自動的に関連するテストを再実行する便利な機能です。開発中にテストを継続的に実行したい場合に役立ちます。
