...
4. テストと必要なドキュメント更新を行います：
   ...
   必要に応じて、Codex 本体の確認に加えてサブエージェントによるレビューも実施します。特に Issue 対応では、issue 本文・関連コメント・現在の差分を入力に、文脈を引き継がない新規サブエージェントだけで技術レビューを行い、指摘を反映した案を別の新規サブエージェントに再レビューさせ、`OK` が出るまで繰り返します。最初の周回で複雑な変更へ広げず、小さな題材でループを検証するときは [Issue 検証ループ](./issue-validation-loop.md) を参照してください。
