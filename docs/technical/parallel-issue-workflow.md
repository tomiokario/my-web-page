...
ここでの並列対応は、「1 つの Issue につき 1 つの作業ブランチ、1 つの worktree、1 つの Codex スレッド」を基本単位にします。親となる進行管理スレッドは Issue の棚卸し、依存関係の整理、実行順序の決定、進捗の集約、merge 後のブランチ削除を担当し、各作業スレッドは割り当てられた Issue の実装、検証、Pull Request 作成、merge 後の worktree 削除までを担当します。
...
この置き方なら、Codex の writable root が本体 repo だけでも、作業スレッドが自分の worktree を編集し、merge 後に削除できます。親 worktree の `git status` で `tmp/` が出ないことを確認してから使います。
...
- 作業スレッド: 1 Issue だけを担当し、専用 worktree と専用ブランチで実装、検証、Pull Request 作成、merge 後の worktree 削除を行う
...
ユーザーから「merge しました」と連絡があったら、各作業スレッドが自分の担当した worktree を後片付けします。進行管理スレッドは、main の最新化、ローカルブランチ削除、リモートブランチ削除、worktree prune、全体の完了確認を行います。

```bash
git -C /path/to/my-web-page checkout main
git -C /path/to/my-web-page pull --rebase origin main
...
`git worktree remove` は作業スレッドで実行します。`git branch -d`、`git push origin --delete`、`git worktree prune` は進行管理スレッドで実行します。`git worktree remove` が `.git/worktrees/` の削除権限で止まる場合は、承認付きで同じコマンドを再実行します。リモートブランチがすでに削除済みの場合は、その結果を確認して次へ進みます。未 merge の worktree や未 push の差分が残っている場合は、作業スレッドが状態を報告して worktree 削除を止めます。
