...
- CI / Vercel / テストの状態
- レビュー待ち
- Codex Review の最終チェック
- rebase 必要
- merge 可能
- merge 後に後続 Issue へ影響する内容

同じ領域を触る Pull Request が複数ある場合は、先に merge するものを決めます。後続 Pull Request は、先行 Pull Request の merge 後に `main` を取り込み、必要な再検証を行います。push 反応型の Codex Review が有効な Pull Request では、fresh review と intent review が `OK` になった差分を push し、妥当な指摘には `+1` reaction を付けて修正後に fresh review と intent review を再実施します。妥当でない指摘には `-1` reaction を付け、人間へ相談します。
