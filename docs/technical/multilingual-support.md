...
3. **useLocale**: 現在の言語に対応する locale オブジェクトを返すカスタムフック
4. **locales**: 各言語のテキストリソースを定義するファイル
5. **言語固有のマークダウンファイル**: 各言語のコンテンツを格納するマークダウンファイル
...
- **型安全な言語設定**: `Language` 型でサポート対象の言語を制限します。
- **ローカルストレージとの連携**: ユーザーの言語設定をローカルストレージに保存し、ページ再読み込み後も設定を維持します。
- **明示的な更新関数**: `setLanguage` と `toggleLanguage` を提供します。
...
`src/locales/types.ts` で locale の共通スキーマを定義しています。日本語・英語の両方が同じ構造を満たすため、文言の追加漏れを防ぎやすくなります。
...
コンポーネント内では `useLocale` フックを使って、現在の言語に対応する locale を取得して使用します。`useLanguage` は主に言語の切り替えや現在値の参照に使います。

## 言語切り替えボタンの実装

ヘッダーコンポーネントなどでは `useLanguage` フックから `toggleLanguage` 関数を取得し、ボタンのクリックイベントなどで呼び出すことで言語を切り替えます。ナビゲーションやサブヘッダーの文言は `routes.tsx` のメタデータと `useLocale` を組み合わせて解決します。

## 言語固有のマークダウンコンテンツ

マークダウンコンテンツも多言語対応しています。詳細は[マークダウンコンテンツ](../admin/markdown-content.md)のドキュメントを参照してください。
...
4. `src/types.ts` の `Language` 型に新しい言語を追加します。
5. `LanguageContext.tsx` の切り替えロジックを必要に応じて更新します。
