...
- 多言語対応（日本語/英語）
- warm-gray base + ice accent をデフォルトにしたテーマ切り替え
- マークダウンベースのコンテンツ管理
- 出版物データの表示と管理
- レスポンシブデザイン
...
サイトテーマは `src/styles/variables.css` の semantic token で管理します。デフォルトは `:root` の warm-gray base + ice accent 系、現行グレー系は `:root[data-theme="classic"]` です。新しいテーマを追加する場合は、React コンポーネントを分けずに同ファイルへ `data-theme` ごとの token override を追加し、`src/contexts/SiteThemeContext.tsx` の許可値と `src/components/ThemeToggle.tsx` の表示だけを更新してください。
