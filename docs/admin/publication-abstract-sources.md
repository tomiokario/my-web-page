# 抄録候補ソース

2026年3月15日時点で、Web 検索で abstract 原文を確認できた publication データのソースを整理します。

このページは `src/data/publication_master.json` の `fields.description` を確認・補完するときの補助メモです。出版物データの正本と更新手順は [出版物データ管理](./publications-management.md) を参照してください。

- `Numerical simulations on optoelectronic deep neural network hardware based on self-referential holography`
  - Source: [10.1007/s10043-023-00810-2](https://doi.org/10.1007/s10043-023-00810-2)
  - Mirror: [Kyutech Repository PDF](https://kyutech.repo.nii.ac.jp/record/2000133/files/neuro_58.pdf)
- `Dependence of activation function on image recognition accuracy in self-referential holographic deep neural network`
  - Source: [Optica / ISOM-ODF 2022 abstract](https://opg.optica.org/abstract.cfm?uri=ISISOM-2022-ITuPJ_01)
- `Numerical Simulations of Optoelectronic AI Hardware using Transmission Matrix of Real Object`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2000113)
  - PDF: [neuro_47.pdf](https://kyutech.repo.nii.ac.jp/record/2000113/files/neuro_47.pdf)
- `Self-referential holographic neural network with single spatial light modulator`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2000111)
  - PDF: [neuro_46.pdf](https://kyutech.repo.nii.ac.jp/record/2000111/files/neuro_46.pdf)
- `Effective training method using transmission matrix for optoelectronic deep neural network`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2000785)
  - PDF: [neuro_68.pdf](https://kyutech.repo.nii.ac.jp/record/2000785/files/neuro_68.pdf)
- `Numerical simulation on wavefront transformation using self-referential holographic deep neural network`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2000786)
  - PDF: [neuro_67.pdf](https://kyutech.repo.nii.ac.jp/record/2000786/files/neuro_67.pdf)
- `Numerical analysis on contributions of individual layers in self-referential holographic deep neural network`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2001193)
  - PDF: [10443741.pdf](https://kyutech.repo.nii.ac.jp/record/2001193/files/10443741.pdf)
- `Numerical simulations on image recognition by self-referential holographic deep neural network with electronic output layer`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2001192)
  - PDF: [10443740.pdf](https://kyutech.repo.nii.ac.jp/record/2001192/files/10443740.pdf)
- `Numerical Simulations of Optoelectronic Deep Neural Network Using Trainable Activation Function`
  - Source: [ISOM 2024 technical program PDF](https://isom.jp/tech_pro_tu2024.pdf)
- `Self-referential holographic data storage with integrated denoising function by deep learning`
  - Source: [ISOM 2024 technical program PDF](https://isom.jp/tech_pro_mon2024.pdf)
- `Numerical simulation of deep reinforcement learning using self-referential holographic deep neural network`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2001766)
- `Experiments on image recognition by optelectronic deep neural network with scattering medium insertion`
  - Source: [ITE paper page](https://www.ite.or.jp/ken/paper/20250219kANt/)
- `Numerical Simulation of Output Filter Design for Optoelectronic Deep Neural Network Using Hyperparameter Optimization`
  - Source: [Kyutech Repository record](https://kyutech.repo.nii.ac.jp/records/2001779)
  - PDF: [neuro_111.pdf](https://kyutech.repo.nii.ac.jp/record/2001779/files/neuro_111.pdf)
- `Numerical simulations on self-referential holographic data storage with built-in denoising function by self-referential holographic deep neural network`
  - Source: [10.1007/s10043-025-00978-9](https://doi.org/10.1007/s10043-025-00978-9)
- `Numerical Simulations of Optoelectronic Neural Network Using Off-Axis Binary Holograms on Digital Micromirror Device`
  - Source: [ISOM 2025 technical program PDF](https://isom.jp/tech_pro_tu2025.pdf)
- `Numerical Simulation of Autonomous Mask Design for Optoelectronic Deep Neural Network by Hyperparameter Optimization`
  - Source: [IWH 2025 book program PDF](https://www.iwh2025.org/uploads/1/5/2/6/152641567/_ooo_1027_book_program.pdf)
- `Numerical simulations on denoising capability of self-referential holographic data storage with built-in denoising function`
  - Source: [IWH 2025 book program PDF](https://www.iwh2025.org/uploads/1/5/2/6/152641567/_ooo_1027_book_program.pdf)
- `Numerical Simulations of Parallel Image Recognition with Self-Referential Holographic Deep Neural Network`
  - Source: [IWH 2025 book program PDF](https://www.iwh2025.org/uploads/1/5/2/6/152641567/_ooo_1027_book_program.pdf)

上記のうち原文を確認できた項目だけを `publication_master.json` へ反映します。ITE の一部ページは `Not available yet` だったため、後日 `fields.description` を補完するときに再確認します。

著作権配慮のため、PDF 等から抽出した長文の verbatim abstract はそのまま転載せず、公開ページに載せる必要がある場合は短い要約または公開元へのリンクで扱います。
