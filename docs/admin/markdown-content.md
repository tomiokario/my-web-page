...
マークダウンファイルは `public/markdown` ディレクトリに配置されています。通常運用では `public/markdown/ja/` と `public/markdown/en/` 配下を正本として扱い、ルート直下の `home.md` や `profilecv.md` は後方互換用のフォールバックとしてのみ扱います。
...
1. 対応する言語のディレクトリ内のマークダウンファイルを編集します。
   ...
   - ルート直下の `public/markdown/*.md` は、言語別ファイルが存在しないケースの後方互換用です。通常の更新対象にはせず、必要性が明確な場合のみ編集してください。
