...
マークダウンファイルは `public/markdown` ディレクトリに配置されています。現在の正本は `public/markdown/ja/` と `public/markdown/en/` 配下の言語別ファイルです。ルート直下の Markdown は使用せず、ローダーもルート直下へのフォールバックを行いません。
...
- **言語別ファイルの読み込み**: `public/markdown/ja/` または `public/markdown/en/` 配下のファイルだけを読み込みます。対象言語のファイルが存在しない場合はエラー表示になります。
...
1. 対応する言語のディレクトリ内のマークダウンファイルを編集します。
   ...
   - 新しいページを追加する場合は、同じ相対パスで日本語版と英語版の Markdown を作成してください。
...
- ファイルが存在しない場合にルート直下へフォールバックせず、エラー表示になるか
- エラーハンドリングが適切に行われるか
- サブディレクトリを含むパスのマークダウンファイルが正しく読み込まれるか
- 複数階層のサブディレクトリを含むパスのマークダウンファイルが正しく読み込まれるか

テストの詳細については、[テスト戦略](../technical/testing-strategy.md)のドキュメントを参照してください。
