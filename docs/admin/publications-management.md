...
日常運用では `publication_master.json` を editor か researchmap export JSONL で更新し、`publications.json` はそこから再生成します。researchmap で修正した内容を戻す場合も、researchmap export JSONL から `publication_master.json` に取り込みます。
...
```mermaid
flowchart LR
    A[Local bridge editor<br/>npm run publications-editor] --> C[Master Data<br/>src/data/publication_master.json]
