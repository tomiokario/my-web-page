...
GitHub で作業ブランチの PR が `main` にマージされると、Vercel が自動的に本番デプロイを実行します。

1. ローカルで変更を加える
2. 作業ブランチでテストと必要な確認を済ませる
3. fresh review と intent review を通し、最新差分が `OK` であることを確認する
4. 変更をコミットし、`git status` で差分を確認してから作業ブランチに push する
5. 日本語の PR を作成し、push 反応型の Codex Review が有効な場合は PR 上の最終チェックとして監視してから `main` にマージする
6. Vercel が自動的にビルド・デプロイを実行する
7. デプロイ完了通知を確認する
...
ロールバックが必要な場合も、原則として作業ブランチで revert を作成し、PR 経由で `main` に反映します。
