...
Codex Review コメントへの対応で修正した場合は、該当する review thread へ返信し、何を直したか、何を確認したか、残課題があるかを自然文で伝える。Codex Review への対応とは別に見つけた独自の追加修正や補足は、既存 review thread へぶら下げず、新規 PR コメントとして投稿する。対外コメントでは内部の role 名や進行用語を出さない。
