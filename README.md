...
- Issue 対応や運用変更では、push / PR 更新の前に fresh review と intent review を通し、PR 上の Codex Review はその後の最終チェックとして扱います
...
- テスト実行
  ...
  npm test
  ```
  - メモ: 出版物変換スクリプトのテストは一時ディレクトリ内の fixture を使うため、tracked data の `src/data/publications.json` は生成/上書きしません。
